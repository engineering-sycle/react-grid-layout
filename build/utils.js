...
/**
 * Compare 2 layouts only by values that we keep in the layout state. This allows
 * the props layout to contain other properties without thrashing the layout.
 */
function compareLayouts(firstLayout, secondLayout) {
  // If different lengths -- return false
  if (firstLayout.length !== secondLayout.length) return false;

  for (var i = 0; i < firstLayout.length; i++) {
    var equal = (0, _lodash2.default)(
      cloneLayoutItem(firstLayout[i]),
      cloneLayoutItem(secondLayout[i])
    );
